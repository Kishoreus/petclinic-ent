...
If `kubectl apply` fails with this, the IAM identity running kubectl isn't
authorized on the cluster. Fixed here two ways:
1. `modules/eks` sets `enable_cluster_creator_admin_permissions = true`, so whoever's IAM identity creates the cluster automatically gets admin.
2. `modules/iam` explicitly grants cluster-admin (via EKS Access Entries) to `data.aws_caller_identity.current.arn` — the IAM user your `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` secrets belong to — plus anything you add to `extra_admin_principal_arns` in `terraform.tfvars` (e.g. your own IAM user, if you want `kubectl` access from your laptop too).

## ⚠️ Free-tier account restriction
New AWS accounts often block launching any non-free-tier instance type
(`t3.medium`, etc.) until the account is further verified/aged. If you hit:
`InvalidParameterCombination - The specified instance type is not eligible
for Free Tier`, that's this restriction — not a bug. This repo now defaults
to `t3.micro` (free-tier eligible) with 3 nodes to compensate for the small
per-node capacity. It's tight (1GB RAM/node) for ~7 Java services, so expect
some pods to be pending/evicted under load — bump `desired_size`/`max_size`
in `terraform.tfvars` if you see that, or ask AWS Support to lift the
restriction once your account is verified, then switch back to `t3.small`/
`t3.medium` for a much more comfortable fit.
...
terraform/
  modules/
    ...
    iam/           -> EKS Access Entries: grants IAM principals cluster access (fixes "kubectl asked for credentials")
  environments/
